    ...
    fetchCurrentUser()
      .then(async ({ data }) => {
        ...
        if (_isMounted) {
          ...
          const skip = 0;
          const limitNotifications = +process.env
            .REACT_APP_NOTIFICATIONS_PER_PAGE;
          ...
          const { data: friendsData } = await fetchFriends({
            skip,
            limit: limitFriends,
          });
        ...
        <Suspense fallback={<div>Loading...</div>}>
          <Switch>
            ...
            <Route
              path="/auth"
              render={(props) => (
                <AuthPage setIsAuth={(value) => setIsAuth(value)} {...props} />
              )}
            />
